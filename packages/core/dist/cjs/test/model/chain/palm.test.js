...
describe('PalmTestnet Chain', function () {
    it('getChainId', function () {
        chai_1.expect(src_1.PalmTestnet.chainId).to.equal(11297108099);
    });
    it('getChainName', function () {
        chai_1.expect(src_1.PalmTestnet.chainName).to.eq('Palm Testnet');
    });
    it('isTestChain', function () {
        chai_1.expect(src_1.PalmTestnet.isTestChain).to.be["true"];
    });
    it('isLocalChain', function () {
        chai_1.expect(src_1.PalmTestnet.isLocalChain).to.be["false"];
    });
    it('getExplorerAddressLink', function () {
        chai_1.expect(src_1.PalmTestnet.getExplorerAddressLink(defaults_1.TEST_ADDRESS)).to.eq("https://explorer.palm-uat.xyz/address/" + defaults_1.TEST_ADDRESS);
    });
    it('getExplorerTransactionLink', function () {
        chai_1.expect(src_1.PalmTestnet.getExplorerTransactionLink(defaults_1.TEST_TX)).to.eq("https://explorer.palm-uat.xyz/tx/" + defaults_1.TEST_TX);
    });
});
